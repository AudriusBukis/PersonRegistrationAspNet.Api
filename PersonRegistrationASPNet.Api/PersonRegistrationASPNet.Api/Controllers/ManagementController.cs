...
namespace PersonRegistrationASPNet.Api.Controllers
{
    ...
    public class ManagementController : Controller
    {
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "Admin, User")]
        public ActionResult<ResponseDto> ChangeHauseNumber([FromQuery] Guid userId, InputIntDto hauseNumber)
        {
            var response = _managementService.ChangeUserHauseNumber(hauseNumber.Number, userId);
            if (!response.IsSuccess)
                return BadRequest(response.Message);
            return response;
        }
