...
namespace PersonRegistrationASPNet.Database.Repositories
{
    public class ManagementRepository : IManagementRepository
    {
        ...
        public ResponseDto SaveUserInfo(UserInfo userInfo, Guid userId)
        {
            try
            {
                ...
                else
                {
                    ...
                    user.UserInfo.Address!.HauseNumber = userInfo.Address!.HauseNumber;
        ...
        public ResponseDto ChangeUserHauseNumber(int hauseNumber, Guid userId)
        {
            try
            {
                var user = GetUserInfo(userId);
                if (user is null)
                    return new ResponseDto(false, "Bad user Id");
                user.UserInfo!.Address!.HauseNumber = hauseNumber;
                _context?.SaveChanges();
                return new ResponseDto(true, "user info was saved");
            }
            catch (Exception ex)
