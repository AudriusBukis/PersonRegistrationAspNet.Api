...
namespace PersonRegistrationASPNet.Tests
{
    public class ManagementControllerTest
    {
        ...
        [Theory, AutoData]
        public void ManagementController_ChangeHauseNumber_ResposeDto_Returns_IsSuccess_True(Guid userId, InputIntDto hauseNumber)
        {
            var managementServiceMoq = new Mock<IManagementService>();
            var sut = new ManagementController(managementServiceMoq.Object);
            managementServiceMoq.Setup(x => x.ChangeUserHauseNumber(It.IsAny<int>(), It.IsAny<Guid>())).Returns(new ResponseDto(true, "message"));

            var response = sut.ChangeHauseNumber(userId, hauseNumber);
            var isSuccess = response.Value?.IsSuccess;

            Assert.True(isSuccess);
        }
        [Theory, AutoData]
        public void ManagementController_ChangeHauseNumber_Returns_BadRequest_Type_Name(Guid userId, InputIntDto hauseNumber)
        {
            var managementServiceMoq = new Mock<IManagementService>();
            var sut = new ManagementController(managementServiceMoq.Object);
            managementServiceMoq.Setup(x => x.ChangeUserHauseNumber(It.IsAny<int>(), It.IsAny<Guid>())).Returns(new ResponseDto(false, "message"));
